        ...
        >
          <div className="h-[42px]">
            <div className="relative z-[0] w-max">
              <div
                className={`border absolute bottom-[-6px] left-[-11px] w-[calc(100%_+_16px)] border-[${color[index]}] `}
              ></div>
              <span className="capitalize ">{data.title}</span> {"events"}
            </div>
          </div>
          <div className="grid other-counts-component grid-cols-2 sm:grid-cols-2 gap-3">
            {data.dates.map((e, indexx) => (
              <div
                ...
                title="anchor"
                id="father-anim"
                className={`mx-auto relative overflow-hidden text-center border-component-etc px-6 hover:border-[${
                  color[index] + "91"
                }]`}
              >
                <div
